...
type AppScreen = 'start' | 'quiz' | 'results';
...
function startGame(mode: GameMode) {
  ...
  currentQuestion.value = questions.length > 0 ? questions[0] : null;
...
function nextQuestion() {
  ...
  // Restart timer for next question in timed mode
  if (gameMode.value === 'timed') {
    startTimer();
  }
}
...
<template>
  <div class="min-h-screen bg-gray-100">
    <!-- Start Screen -->
    <div v-if="currentScreen === 'start'" class="min-h-screen flex items-center justify-center p-6">
      <StartScreen @start-game="startGame" />
    </div>
